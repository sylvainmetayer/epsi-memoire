...
Parler du la mise en place après déploiement avec Jenkins + ansible.

Parler du rollback en cas d'échec de déploiement.
...
La gestion de l'infrastructure fait également partie de l'automatisation d'un projet.

\todo{Gestion de l'infrastructure}

avant server bare metal à config a la main

maintenant on décrit ce dont a besoin et on se sert de docker ansible terraform \& pour provisionner des machines virtuelles

\gls{IaC} permet de passer de configuration manuelle à provisionnement automatique. Permet d'éviter les wiki non mis à jour d'infrastructure parce que patch déployé à l'arrache sur le serveur.

Avantage : versionnable donc on peut consulter les modifications et l'évolution. Reproductibilité : automatique donc on peut reproduire à l'identique. facilite provisionnement
...
traitement des logs selon 12factor serait de faire tout vers stdout stderr et laisser les outils de monitoring exécution (docker) gérer l'écriture dans des fichiers.

surveiller trafic avec retour 200 / 400 / 500
