...
Les tests garantissent alors la validité du comportement de l'application. On distingue plusieurs type de tests.

\subsubsection{Les tests unitaire}

L'objectif d'un test unitaire est d'attester le bon fonctionnement d'un composant isolé du reste de l'application. L'objectif est que la fonctionnalité testée soit totalement isolée afin de se concentrer uniquement sur cette dernière, sans dépendre d'autre composant. On peut alors tester tous les cas, du cas dit \frquote{général}, qui représente 80\% des cas, jusqu'aux \frquote{cas limites}, cas apparaissant plus rarement mais pouvant entrainer un comportement non souhaité de l'application.
...
\missingfigure{Figure intégration failed}
